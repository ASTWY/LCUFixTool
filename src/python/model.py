...
def Set_or_Get_wad_state(lolpath: str, wadinfo, model="GET", downhost=""):
    ...
    state = False  # 当前模块状态
    _wad = os.path.join(
        lolpath, os.path.join(os.path.dirname(wadinfo["path"]), wadinfo["name"])
    )  # wad文件路径
    # 如果wad文件不存在
    ...
    # 载入配置json文件
    data = json.load(
        open(os.path.join(lolpath, wadinfo["path"]), "r", encoding="utf-8")
    )["riotMeta"]
    ...
    if model == "GET":
        ...
        if wadinfo["type"] == "cover":
            ...
            if wadinfo["name"] in data["globalAssetBundles"]:
                data["globalAssetBundles"].remove(wadinfo["name"])
    with open(os.path.join(lolpath, wadinfo["path"]), "r+", encoding="utf-8") as f:
        _tmp1 = json.load(f)
        _tmp1["riotMeta"] = data
        f.seek(0)
        f.truncate()
        f.write(json.dumps(_tmp1, ensure_ascii=False, indent=4, sort_keys=True))
