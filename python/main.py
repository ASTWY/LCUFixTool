...
if __name__ == '__main__':
    ...
    while True:
        if not os.path.exists(os.path.join(_path, "LeagueClient\\Plugins")):
            ...
            if config.get('lol') != _path:
                config['lol'] = _path
                with open(os.path.join(cfg_path, "config.json"),
                          "w+",
                          encoding="utf-8") as f:
                    f.seek(0)
                    f.truncate()
    ...
    if _ver != cfg['ver']:
        col.print("[bold cyan]{}[/bold cyan][bold red]暂不支持[/bold red]，请耐心等待更新".
                  format(_ver))
        os.system('pause')
    else:
        while True:
                ...
                for p in _tmpList['data']:
                    ...
                    if not _tstate:
                        for _titem in _tmpList['data']:
                            Set_or_Get_wad_state(lolpath=_path,
                                                 wadinfo=_titem,
                                                 model="DEL")
            ...
            table.add_row(
                str(i),
                str("[green]√\t[/green]" if (
                    not DelorRe_TX(_path)) else "[red]×\t[/red]"), "净化客户端",
